...
namespace DeedsOfUtgard
{
    ...
    public static class RegisterAndCheckVersion
    {
        private static void Prefix(ZNetPeer peer, ref ZNet __instance)
        {
            ...
            peer.m_rpc.Register($"{DeedsOfUtgardPlugin.ModName}_VersionCheck",
                new Action<ZRpc, ZPackage>(RpcHandlers.RPC_DeedsOfUtgard_Version));

            // Make calls to check versions
            DeedsOfUtgardPlugin.DeedsOfUtgardLogger.LogInfo("Invoking version check");
    ...
    public static class VerifyClient
    {
        private static bool Prefix(ZRpc rpc, ZPackage pkg, ref ZNet __instance)
        {
            ...
            DeedsOfUtgardPlugin.DeedsOfUtgardLogger.LogWarning(
    ...
    public static class RpcHandlers
    {
        ...
        public static void RPC_DeedsOfUtgard_Version(ZRpc rpc, ZPackage pkg)
        {
            ...
            DeedsOfUtgardPlugin.DeedsOfUtgardLogger.LogInfo("Version check, local: " +
                                                            ...
                                                            ",  remote: " + version);
            if (version != DeedsOfUtgardPlugin.ModVersion)
            {
                DeedsOfUtgardPlugin.ConnectionError =
                    $"{DeedsOfUtgardPlugin.ModName} Installed: {DeedsOfUtgardPlugin.ModVersion}\n Needed: {version}";
            ...
            else
            {
                if (!ZNet.instance.IsServer())
                {
                    ...
                    DeedsOfUtgardPlugin.DeedsOfUtgardLogger.LogInfo("Received same version from server!");
                }
                else
                {
